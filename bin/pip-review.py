...
def parse_args():
    ...
    parser.add_argument('--send', '-s', action='store_true', default=False,
                        help='Sends emails with updates available for packages ')
...
def send_email_with_new_packages(pkgs):
    # only send email if there are updates
    if pkgs:
        project_name = get_project_name()
        body = "\n".join(pkgs)
        msg = MIMEText(body)
        msg['From'] = 'pip-new@letolab.com'
        msg['To'] = 'martin@letolab.com'
        ...
        mailserver.sendmail(mail_user, 'martin@letolab.com', msg.as_string())
        mailserver.quit()


def get_project_name():
    path = check_output('echo $PATH')
    project_name = path.split(':')[0].split('/')[-3]
    return project_name.capitalize()


def main():
    args = parse_args()
    setup_logging(args.verbose)
    pkgs_with_new_versions = []
        ...
        if latest_version is None:
            ...
            if args.raw:
                ...
                if args.auto:
                    ...
                    logging.info('%s==%s is available (you have %s)' % (
                        pkg,
                        latest_version, installed_version))
                    pkgs_with_new_versions.append(
    ...
    if args.send:
        send_email_with_new_packages(pkgs_with_new_versions)
